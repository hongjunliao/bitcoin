...
/////////////////////////////////////////////////////////////////////////////////////////////
#ifndef BTC_SERIALIZE_H
#define BTC_SERIALIZE_H
...
public:
    ...
    {
        if (pstr == NULL){
            fprintf(stderr, "CFixedFieldString::Unserialize : trying to unserialize to const string");
            return;
        }
        char pszBuf[LEN+1];
        s.read(pszBuf, LEN);
...
void Unserialize_impl(Stream& is, std::vector<T, A>& v, int nType, int nVersion, const boost::true_type&)
{
    ...
    while (i < nSize)
    {
        unsigned int blk = std::min((size_t)(nSize - i), (size_t)(1 + 4999999 / sizeof(T)));
        v.resize(i + blk);
