...
{
    CDB reviewdb("reviews.dat", "r+", false);
    ...
    for (int side = 0; !pmapPropagate[side].empty(); side = 1 - side)
    {
        ...
        for (map<uint256, vector<unsigned short> >::iterator mi = mapFrom.begin(); mi != mapFrom.end(); ++mi)
        {
            ...
            // Read user
            CUser user;
            reviewdb.Read(make_pair(string("user"), hashUser), user);
            ...
            // Write back
            if (!reviewdb.Write(make_pair(string("user"), hashUser), user))
                return false;
        }
        mapFrom.clear();
    }
...
bool CReview::AcceptReview()
{
    ...
    reviewdb.Read(make_pair(string("reviews"), hashTo), vReviews);
    vReviews.push_back(*this);
    if (!reviewdb.Write(make_pair(string("reviews"), hashTo), vReviews))
        return false;

    // Add link from sender
    CUser user;
    uint256 hashFrom = Hash(vchPubKeyFrom.begin(), vchPubKeyFrom.end());
    reviewdb.Read(make_pair(string("user"), hashFrom), user);
    user.vLinksOut.push_back(hashTo);
    if (!reviewdb.Write(make_pair(string("user"), hashFrom), user))
        return false;
...
bool CProduct::CheckProduct()
{
    ...
    CDB reviewdb("reviews.dat", "r", false);
    CUser user;
    reviewdb.Read(make_pair(string("user"), GetUserHash()), user);
